...
import com.lit.kcustomercore.extension.edit
import com.lit.kcustomercore.extension.sharedPreferences
import java.util.*
...
object GlobalUtil {
    ...
    val appPackage: String
        get() = LincApplication.context.packageName
     ...
     */
    val appName: String
        get() = LincApplication.context.resources.getString(LincApplication.context.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = LincApplication.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            LincApplication.context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            LincApplication.context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getString(resId: Int): String {
        return LincApplication.context.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return LincApplication.context.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return LincApplication.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = LincApplication.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            LincApplication.context.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = LincApplication.context.packageManager
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(LincApplication.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
